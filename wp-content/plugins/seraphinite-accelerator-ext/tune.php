...
function _SelfDiag_GetResponseResString( $url, $requestRes, $body = false )
{
	if( is_wp_error( $requestRes ) )
		return( sprintf( Wp::safe_html_x( 'RequestWpErr_%1$s%2$s', 'admin.SelfDiag', 'seraphinite-accelerator' ), $url, $requestRes -> get_error_message() ) );

	$res = 'HTTP ' . ( isset( $requestRes[ 'method' ] ) ? ( $requestRes[ 'method' ] . ' ' ) : '' ) . wp_remote_retrieve_response_code( $requestRes ) . ' ' . $url;
	$res .= '<br>' . GetHeadersResString( array( 'Server Address' => $_SERVER[ 'SERVER_ADDR' ] ) );
...
function OnAdminApi_SelfDiag_AsyncRequest( $args )
{
	...
	else
	{
		...
		if( $res -> hr !== Gen::S_OK && Gen::GetArrField( Plugin::SettGet(), array( 'asyncUseCron' ), true ) )
...
function OnAdminApi_SelfDiag_PageOptimize( $args )
{
	...
	else if( wp_remote_retrieve_body( $requestRes ) !== 'selfTest-' . $selfTestSuffix )
	{
...
function OnAdminApi_SelfDiag_ExtCache( $args )
{
	...
	else if( wp_remote_retrieve_header( $requestRes, 'X-Seraph-Accel-test' ) !== $tmStamp && !$isExtCacheAllowed )
	{
...
function SelfDiag_DetectStateAnd3rdPartySettConflicts( $cb, $ext = false )
{
	...
	if( $isCacheEnabled )
	{
		...
		if( !isset( $sett[ PluginOptions::VERPREV ] ) && ( (isset($_SERVER[ 'REQUEST_METHOD' ])?$_SERVER[ 'REQUEST_METHOD' ]:null) == 'GET' ) && !CacheVerifyEnvDropin( $sett ) )
		{
			if( !@file_exists( WP_CONTENT_DIR . '/advanced-cache.php' ) && !@is_writable( WP_CONTENT_DIR ) )
				...
				call_user_func_array( $cb, array( Ui::MsgErr, sprintf( Wp::safe_html_x( 'ContentDropinNotMatch_%1$s%2$s', 'admin.Notice', 'seraphinite-accelerator' ), Gen::GetFileName( WP_CONTENT_DIR ), 'advanced-cache.php' ) ) );
		}
	...
	if( $isContProcEnabled )
	{
		...
		foreach( array( 'webp','avif' ) as $comprType )
		{
			...
			$dataTest = @file_get_contents( __DIR__ . '/Images/Test.png' );
			$optionText = sprintf( Plugin::GetPluginString( 'SubjectTitle_%1$s%2$s' ), esc_html_x( 'Lbl', 'admin.Settings_Images_Compr', 'seraphinite-accelerator' ), Gen::GetArrField( array( 'avif' => sprintf( esc_html_x( 'AvifChk_%1$s%2$s', 'admin.Settings_Images_Compr', 'seraphinite-accelerator' ), '', '' ), 'webp' => sprintf( esc_html_x( 'WebpChk_%1$s', 'admin.Settings_Images_Compr', 'seraphinite-accelerator' ), '' ) ), $comprType, '' ) );

			$testUrl = add_query_arg( array( '_' => Gen::MicroTimeStamp() ), plugins_url( '/Images/Test.png', __FILE__ ) );
			$requestRes = Wp::RemoteGet( $testUrl, array( 'timeout' => 30, 'sslverify' => false, 'headers' => array( 'Accept' => 'image/' . $comprType . ',image/*,*/*;q=0.8' ) ) );
			if( Net::GetHrFromWpRemoteGet( $requestRes ) !== Gen::S_OK )
			{
			...
			else
			{
				...
				if( $data )
				{
					if( $data !== @file_get_contents( __DIR__ . '/Images/Test.png.' . $comprType ) )
					{
						if( $data === $dataTest )
							call_user_func_array( $cb, array( Gen::SevWarn, vsprintf( Wp::safe_html_x( 'ImgConvRedir_NotActive_%1$s%2$s%3$s%4$s', 'admin.Notice', 'seraphinite-accelerator' ), array_merge( Ui::Link( array( '', '' ), Plugin::RmtCfgFld_GetLoc( $rmtCfg, 'Help.Settings_Images_Compr' ), true, array(  ) ), array( $comprType, $optionText ) ) ) ) );

					}
				}
			}

			$hr = Img::ConvertDataEx( $dataCnv, $dataTest, 'image/' . $comprType );
			if( Gen::HrFail( $hr ) )
