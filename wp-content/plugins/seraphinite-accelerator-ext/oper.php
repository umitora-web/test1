...
function CacheOpGetViewsHeaders( $settCache, $viewId = null )
{
	...
	if( $viewId === null || $viewId === 'cmn' )
		$res[ 'cmn' ] = array( 'User-Agent' => 'Mozilla/99999.9 AppleWebKit/9999999.99 (KHTML, like Gecko) Chrome/999999.0.9999.99 Safari/9999999.99 seraph-accel-Agent/2.19.7' );
...
function CacheVerifyEnvDropin( $sett )
{

	return( @file_get_contents( WP_CONTENT_DIR . '/advanced-cache.php' ) == GetAdvCacheFileContent( $sett ) );
...
function CacheInitEnv( $sett, $init = true )
{
	...
	$imgTypesCnvFrom_RegExpEnum = implode( '|', array( 'jpe','jpg','jpeg','png','gif','bmp' ) );
	if( Gen::HtAccess_IsSupported() )
	{
		$htaccessBlock = '';
		...
		foreach( array_reverse( array( 'webp','avif' ) ) as $comprType )
		{
			...
			$htaccessBlock .=
				...
				( Gen::GetArrField( $sett, array( 'hdrTrace' ), false ) && !( @preg_match( '@IdeaWebServer@i', (isset($_SERVER[ 'SERVER_SOFTWARE' ])?$_SERVER[ 'SERVER_SOFTWARE' ]:'') ) ) ? "\t\t" . 'Header set X-Seraph-Accel-Cache "2.19.7; state=preoptimized;"' . "\n" : '' ) .
				"\t" . '</FilesMatch>' . "\n" .
				'</IfModule>' . "\n" .
				'';
		}
...

		{
			...
			if( in_array( 'brotli', $dataComprs ) )
			{
				$htaccessBlock .=
					...
					( Gen::GetArrField( $sett, array( 'hdrTrace' ), false ) && !( @preg_match( '@IdeaWebServer@i', (isset($_SERVER[ 'SERVER_SOFTWARE' ])?$_SERVER[ 'SERVER_SOFTWARE' ]:'') ) ) ? "\t\t" . 'Header set X-Seraph-Accel-Cache "2.19.7; state=precompressed;"' . "\n" : '' ) .
					"\t" . '</FilesMatch>' . "\n" .
					'</IfModule>' . "\n" .
					'';
			}
...

	{
		...
		foreach( array_reverse( array( 'webp','avif' ) ) as $comprType )
		{
			...
			$confComprRedirBlock .=
				"\t" . '# ' . $comprType . '' . "\n\t" . 'types { image/' . $comprType . ' ' . $comprType . '; }' . "\n\t" . 'set $' . $comprType . '_redir "";' . "\n\t" . 'if ($http_accept ~* "image\\/' . $comprType . '") { set $' . $comprType . '_redir "${' . $comprType . '_redir}A"; }' . "\n\t" . 'if (-f $request_filename.' . $comprType . ') { set $' . $comprType . '_redir "${' . $comprType . '_redir}F"; }' . "\n\t" . 'if ($' . $comprType . '_redir = "AF") { add_header Vary Accept; rewrite (.*) $1.' . $comprType . ' break; }' . "\n" .
				'';
		}

		$fileConfComprRedir = get_home_path() . 'seraph-accel-img-compr-redir.conf';
		if( $confComprRedirBlock )
		{
			$confTypesFromBlock = '';
