...
function _ApplyInlinesBack( &$data, array &$inlines )
{
	foreach( array_reverse( $inlines, true ) as $inlineType => $inlineTypeItems )
	{
		...
		foreach( $inlineTypeItems as $inlineTypeItemId => $inlineTypeItem )
		{
			...
			if( $encodeUrl )
				$itemSignature = rawurlencode( $itemSignature );

			$data = str_replace( $itemSignature, $inlineTypeItem, $data );
		}
	}
...
function _NormalizeHtmlData( $norm, &$data, $encoding = null, $min = false )
{
	...
	if( $norm & 1 )
	{
		...
		{
			_MaskTag( $aMaskedTagsLite, array( 'p', 'address', 'ol', 'ul' ), 'block', true );
...
function HtmlDocDeParse( $doc, $norm = 0 )
{
	$inlines = array();
	$idNext = 1;
	...
	if( (isset($doc -> documentElement)?$doc -> documentElement:null) )
	{
