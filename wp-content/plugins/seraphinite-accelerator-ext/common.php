...
{
	return( Gen::GetArrField( Plugin::SettGet(), array( 'asyncUseCron' ), true ) ? 'wp-cron.php' : 'index.php' );
}
		...
		'cache' => array(
			...
			'viewsDeviceGrps' => array(
				...
				array(
					...
					'name' => '',
					'id' => 'mobilehighres',
					'agents' => array( 'android', 'bada', 'incognito', 'kindle', 'maemo', 'mobi', 'opera mini', 's8000', 'series60', 'ucbrowser', 'ucweb', 'webmate', 'webos', ),
				),
				...
				array(
					...
					'name' => '',
					'id' => 'mobile',
					'agents' => array( 'mobile', 'android', 'silk/', 'kindle', 'blackberry', 'opera mini', 'opera mobi', ),
				),
			),
		...
		'contPr' => array(
			...
			'js' => array(
				...
				'other' => array(
					'incl' => array(
						...
						'descendant::iframe[@id=\'likes-master\' and contains(@src,\'//widgets.wp.com/likes/master.html?\')]',
			...
			'css' => array(
				...
				'custom' => array(
					...
					'preloaders'	=> array( 'enable' => true,		'descr' => 'Preloaders',				'data' => "#preloader,#page_preloader,#loader-wrapper,#royal_preloader,#loftloader-wrapper,.rokka-loader,.page-preloader-cover,.apus-page-loading,.medizco-preloder,e-page-transition {\n\tdisplay:none!important;\n}\n\nbody.royal_preloader{\n\tvisibility:hidden!important;\n}" ),
					'elementor-vis'		=> array( 'enable' => true,		'descr' => 'Elementor (visibility and animation)',					'data' => ".elementor-invisible {\n\tvisibility: visible!important;\n}\n\n.elementor-element[data-settings*=\"animation\\\"\"] {\n\tanimation-name: none!important;\n}" ),
					'elementor'		=> array( 'enable' => true,		'descr' => 'Elementor',					'data' => ".vc_row[data-vc-full-width-init=\"false\"] {\n\tposition: relative;\n\twidth: var(--seraph-accel-client-width);\n\tleft: calc((100% - var(--seraph-accel-client-width)) / 2);\n}\n\n.elementor-top-section.elementor-section-stretched[data-settings*=\"section-stretched\"] {\n\twidth: var(--seraph-accel-client-width)!important;\n\tleft: calc((100% - var(--seraph-accel-client-width)) / 2)!important;\n}\n\n.seraph-accel-js-lzl-ing .elementor-headline-dynamic-text.elementor-headline-text-active {\n\topacity: 1;\n}" ),
				...
				'items' => array(
					...
					'desktop' => array(
						...
						'contPr' => array(
							...
							'js' => array(
								'optLoad' => true,
								'nonCrit' => array( 'timeout' => array( 'enable' => true, 'v' => 4500 ) ),
...
function CacheCw( $settCache, $siteRootPath, $dataPath, $composite, $content, $type, $fileExt = null )
{
	...
	if( $type != 'html' )
	{
		...
		if( !$composite )
		{
			if( ( $type != 'css' && $type != 'js' ) || IsSrvNotSupportGzAssets() )
				$dataComprs = array( '' );
...
{
	if( !(isset($sett[ 'debug' ])?$sett[ 'debug' ]:null) || !isset( $_REQUEST[ 'seraph_accel_proc' ] ) )
		return( null );
	return( !!$_REQUEST[ 'seraph_accel_proc' ] );
}

function ApplyContentProcessorForceSett( &$sett )
{
	$settContPrOverride = Gen::SanitizeTextData( $_REQUEST[ 'seraph_accel_proc' ] );
...
function ParseContCachePathArgs( $serverArgs, &$args )
{
	...
	if( $posQuery !== false )
	{
		...
		if( $queryOrig != (isset($serverArgs[ 'QUERY_STRING' ])?$serverArgs[ 'QUERY_STRING' ]:null) )
		{
			$args = array();
...
function ContProcGetSkipStatus( $content )
{
	...
	if( $http_response_code !== 200 )
		return( $seraph_accel_g_contProcGetSkipStatus = 'httpCode:' . $http_response_code );
...
function MatchUserAgentExpressions( $userAgent, $aAgent )
{
...
{
	return( 'Mozilla/99999.9 AppleWebKit/9999999.99 (KHTML, like Gecko) Chrome/999999.0.9999.99 Safari/9999999.99 seraph-accel-Agent/2.19.7 ' . ucwords( implode( ' ', Gen::GetArrField( $viewsDeviceGrp, array( 'agents' ), array() ) ) ) );
}
...
function Queue_IsPriorFirst( $siteId, $priority )
{
	...
	foreach( array( 1, 0 ) as $state )
	{
		...
		if( Gen::GetArrField( current( $a -> slice( 0, 1 ) ), array( 'p' ), 0 ) == $priority )
		{
			$res = true;
			break;
		}
...
function ProcessCtlData_Update( $fileCtl, $data, $clearPrev = false, $lifeCtlOpenClose = null )
{
	...
	if( $lifeCtlOpenClose === true )
	{
		...
		@fclose( $seraph_accel_g_hFileCtl );
...
function OnAsyncTask_CacheProcessItem( $args )
{
	...
	if( $urlRedir = trim( wp_remote_retrieve_header( $requestRes, 'location' ) ) )
	{
	...
	if( $skipStatus )
	{
		if( Gen::StrStartsWith( $skipStatus, 'httpCode:' ) && $urlRedir )
...
function GetExtContents( $url, &$contMimeType = null, $userAgentCmn = true, $timeout = 30, $rememberServerState = true )
{
	...
	if( $userAgentCmn )
		$args[ 'user-agent' ] = 'Mozilla/99999.9 AppleWebKit/9999999.99 (KHTML, like Gecko) Chrome/999999.0.9999.99 Safari/9999999.99 seraph-accel-Agent/2.19.7';
...
function CacheExtractPreparePageParams( &$args )
{
	...
	unset( $args[ 'seraph_accel_prep' ] );
	unset( $_GET[ 'seraph_accel_prep' ] );
	unset( $_REQUEST[ 'seraph_accel_prep' ] );
	unset( $requestUriArgs[ 'seraph_accel_prep' ] );
	unset( $redirect_query_string_args[ 'seraph_accel_prep' ] );
	unset( $query_string_args[ 'seraph_accel_prep' ] );
