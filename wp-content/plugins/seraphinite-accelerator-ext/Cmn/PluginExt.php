...
class PluginLic
{
	...
	static function Activate( $key )
	{
		...
		$licRes = LicCli::Action( $srvUrl, '594EDF52DE173808712397', LicCli::Act_Activate, $key, 'wordpress-accelerator', '2.19.7', Wp::GetSiteId(), Wp::GetSiteDisplayName() );
	...
	static function Deactivate()
	{
		...
		$licRes = LicCli::Action( $srvUrl, '594EDF52DE173808712397', LicCli::Act_Deactivate, $licInfo[ 'key' ], 'wordpress-accelerator', '2.19.7', Wp::GetSiteId() );
		$licSrvRes = $licRes[ 'response' ];
		if( !$licSrvRes )
			return( $licRes[ 'hr' ] );
	...
	static function Update()
	{
		...
		$licRes = LicCli::Action( $srvUrl, '594EDF52DE173808712397', LicCli::Act_Check, $licInfo[ 'key' ], 'wordpress-accelerator', '2.19.7', Wp::GetSiteId() );
		$licSrvRes = $licRes[ 'response' ];
		if( !$licSrvRes )
			return( $licRes[ 'hr' ] );
	...
	static function GetDataUrl( $item )
	{
		...
		return( LicCli::GetDataUrl( $srvUrl, '594EDF52DE173808712397', $licInfo[ 'key' ], 'wordpress-accelerator', '2.19.7', Wp::GetSiteId(), $features[ 0 ], $item ) );
	}
...
class PluginExt
{
	...
	static function GetOrderContent()
	{
		...
		$res = '';
		$res .= Ui::Button( esc_html_x( 'OrderBtn', 'admin.Common_SwitchTo', 'seraphinite-accelerator' ), true, false, 'ctlSpaceAfter ctlVaMiddle', 'button', array( 'onclick' => 'window.open( \'' . Plugin::RmtCfgFld_GetLoc( $rmtCfg, 'Links.UrlProductBuy' ) .  '\', \'_blank\' );' ) );
	...
	static function GetSettingsLicenseContent()
	{
...

		{
...

			{
				...
				{
					$res .= Ui::SettBlock_ItemSubTbl_Begin( array( 'class' => 'std', 'style' => array( 'width' => '100%' ) ) );
					{
						$res .= Ui::TagOpen( 'tr' );
						{
							if( !$licKey )
								$res .= Ui::Tag( 'td', Ui::TextBox( 'seraph_accel_licKey', Lic::GetKeyDisplayText( $licKey ), array( 'placeholder' => 'Enter license key', 'style' => array( 'width' => '100%' ) ), true ), array( 'style' => array( 'width' => '100%' ) ) );
							$res .= Ui::Tag( 'td', Ui::Button( $licKey ? esc_html_x( 'DeactivateBtn', 'admin.Common_Lic', 'seraphinite-accelerator' ) : esc_html_x( 'ActivateBtn', 'admin.Common_Lic', 'seraphinite-accelerator' ), false ) );
						}
						$res .= Ui::TagClose( 'tr' );
					}
					$res .= Ui::SettBlock_ItemSubTbl_End();

					{
						$htmlOrderContent = PluginExt::GetOrderContent();
						if( $htmlOrderContent )
							$res .= Ui::Tag( 'div', $htmlOrderContent, array( 'class' => 'ctlSpaceVBefore' ) );
					}
