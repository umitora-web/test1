...
class Lock
{
	...
	function Acquire( $wait = true )
	{
		...
		if( !@flock( $this -> h, ( $wait ? 0 : LOCK_NB ) | LOCK_EX ) )
		{
			$this -> Release();
			return( false );
		}
...
class ArrayOnFiles implements \Iterator, \ArrayAccess, \Countable
{
	...
	function slice( int $offset, $length = null )
	{
		$res = array();
	...
	function splice( int $offset, $length = null )
	{
		$res = array();
...
class HtmlNd
{
	...
	static function CreateTag( $doc, $tag, $attrs = null, $aChildren = null )
	{
		...
		foreach( ( array )$attrs as $attr => $attrVal )
		{
			if( $attr === 'disabled' )
			{
		...
		foreach( ( array )$aChildren as $child )
...
class Wp
{
	...
	{
		return( @tempnam( Wp::GetTempDir(), substr( 'accel', 0, 3 ) ) );
	}
