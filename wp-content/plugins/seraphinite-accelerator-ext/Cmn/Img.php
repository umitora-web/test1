...
class Img
{
	...
	static function GetInfoFromData( $data, $ext = false )
	{
		...
		if( gettype( $data ) == 'string' && @preg_match( '@<svg[>\\s]@i', $data ) && @preg_match( '@</svg>@i', $data ) )
		{
			...
			foreach( array( 'cx' => $ndRoot -> getAttribute( 'width' ), 'cy' => $ndRoot -> getAttribute( 'height' ) ) as $k => $v )
				$info[ $k ] = Gen::StrEndsWith( $v, 'px' ) ? ( int )substr( $v, 0, -2 ) : null;

			if( $info[ 'cx' ] === null && $info[ 'cy' ] === null )
			{

				$viewBox = $ndRoot -> getAttribute( 'viewBox' );
