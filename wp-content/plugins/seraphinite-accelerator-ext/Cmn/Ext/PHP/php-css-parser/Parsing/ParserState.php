...
class ParserState {
	...
	public function parseCharacter($bIsForIdentifier) {
		if ($this->peek() === '\\') {
			if ($bIsForIdentifier && ($this->oParserSettings->bLenientParsing & Settings::ParseErrMed) && ($this->comes('\0') || $this->comes('\9'))) {
	...
	public function consumeShityScope()
	{
		...
		if (!($this->getSettings()->bLenientParsing & Settings::ParseErrMed))
			throw new UnexpectedTokenException('Unexpected \'{\'', '', 'custom', $this->iLineNo);
	...
	public function consumeQuoted($symQoute) {
		$quoted = '';
		if( $symQoute === '"' || $symQoute === '\'' ) {
			while (($char = $this->consume(1)) !== '') {
				$quoted .= $char;
				if ($char === $symQoute && substr($quoted, -2, 1) !== '\\')
					break;
			}
		}
	...
	public function consumeUntil($aEnd, $bIncludeEnd = false, $consumeEnd = false, $consumeQuoted = false, array &$comments = array()) {
		...
		$bCharEscaped = false;
		while (($char = $this->consume(1)) !== '') {
			if (in_array($char, $aEnd) && (strpos('@', $char) === false || !$bCharEscaped)) {
				if ($bIncludeEnd) {
					$out .= $char;
				} elseif (!$consumeEnd) {
