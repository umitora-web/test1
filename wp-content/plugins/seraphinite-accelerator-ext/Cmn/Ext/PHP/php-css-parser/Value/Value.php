...
abstract class Value implements Renderable {
	...
	public static function parseValue(ParserState $oParserState, $aListDelimiters = array(), $bAnyId = false) {
		...
		while (!($oParserState->comes('') || $oParserState->comes('}') || $oParserState->comes(';') || $oParserState->comes('!') || $oParserState->comes(')') || $oParserState->comes('\\'))) {
	...
	public static function parseIdentifierOrFunction(ParserState $oParserState, $bIgnoreCase = false, $bAnyId = false) {
		if ($bAnyId)
			$sResult = trim($oParserState->consumeUntil(array(',', '(', ')', '{', '}', ';', '@'), false, false, true));
		else
	...
	public static function shouldParseCalcFunction(ParserState $oParserState) {
		...
		foreach($g_aCalcFuncs as $f)
			if ($oParserState->comes($f, true))
				return true;
		return false;
	}
	...
	public static function parsePrimitiveValue(ParserState $oParserState, $bAnyId = false) {
		$oValue = null;
		$oParserState->consumeWhiteSpace();
		if (is_numeric($oParserState->peek()) || ($oParserState->comes('-.') && is_numeric($oParserState->peek(1, 2))) || (($oParserState->comes('-') || $oParserState->comes('+') || $oParserState->comes('.')) && is_numeric($oParserState->peek(1, 1)))) {
		...
		} else if ($oParserState->comes("progid:") && ($oParserState->getSettings()->bLenientParsing & Settings::ParseErrMed)) {
