...
class Rule implements Renderable, Commentable {
	...
	public static function parse(ParserState $oParserState) {
		...
		$sIeHackPrefix = '';
		if ($oParserState->comes('*') || $oParserState->comes('_')) {
			$sIeHackPrefix = $oParserState->consume();
		}
		$bVarPrefix = $oParserState->comes('--');
		$oRule = new Rule($oParserState->parseIdentifier(false), $oParserState->currentLine());
	...
	public function render(\seraph_accel\Sabberworm\CSS\OutputFormat $oOutputFormat) {
		...
		if ($this->bIsImportant) {
			$sResult .= $oOutputFormat->space('BeforeImportant') . '!important';
		}
