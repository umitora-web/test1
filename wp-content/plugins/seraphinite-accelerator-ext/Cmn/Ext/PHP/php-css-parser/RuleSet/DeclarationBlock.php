...
class DeclarationBlock extends RuleSet {
	...
	public static function parse(ParserState $oParserState) {
		...
		$selector = $oParserState->consumeUntil(array('{', '}', ';', '@'), false, false, true, $aComments);
		if(!$oParserState->comes('{')) {
			if($oParserState->comes(';'))
				$oParserState->consume();
			throw new SourceException("Unexpected end of selector", $oParserState->currentLine());
		}
		$oParserState->consume();

		$oResult->setSelector($selector);
