...
{
	add_menu_page( Plugin::GetPluginString( 'TitleLong' ), Plugin::GetNavMenuTitle(), 'manage_options', 'seraph_accel_manage',																		$accepted ? 'seraph_accel\\_ManagePage' : 'seraph_accel\\Plugin::OutputNotAcceptedPageContent', Plugin::FileUri( 'icon.png?v=2.19.7', __FILE__ ) );
...
function _InitCatchDataUpdate( $meta = false )
{
	$sett = Plugin::SettGet();
	...
	if( Gen::GetArrField( $sett, array( 'cache', 'updPost' ), false ) )
	{
		...
		register_shutdown_function( 'seraph_accel\\_OnCheckUpdatePost' );
	}
	...
	if( $updGlog )
		register_shutdown_function( 'seraph_accel\\_OnCheckUpdateGlob' );
}
...
function OnInit( $isAdminMode )
{
	...
	if( $isAdminMode )
	{
		...
		if( !$updPostMetaAlways )
			_InitCatchDataUpdate( !$isGet && _IsRequestAjax() );
	...
	if( !$updPostMetaAlways )
	{
		...
		add_filter( 'itglx_wc1c_ignore_catalog_file_processing', function( $ignoreProcessing ) { _InitCatchDataUpdate( true ); return( $ignoreProcessing ); } );
	}
	...
	{
		add_action( 'rest_api_init', function( $wp_rest_server ) { _InitCatchDataUpdate( true ); }, 0, 1 );
		add_action( 'init', function() { _InitCatchDataUpdate( _IsRequestAjax() ); } );
	}
...
function _OnCheckUpdatePost()
{
	...
	if( $seraph_accel_g_postUpdated )
		Plugin::AsyncFastTaskPost( 'CheckUpdatePostProcessAdd', array( 'a' => $seraph_accel_g_postUpdated ), 60 * 60 );
}
...
function CacheInitOperScheduler( $force = false, $init = true )
{
	...
	Plugin::AsyncTaskPost( 'CacheNextScheduledOp', array( 'aId' => $aId ), array( $tmNearest ), false, true );
...
{
	return( LocId::UnPack( $v, function( $id, $comp ) { return( _x( $id, 'admin.' . ( $comp ? ( $comp . '_' ) : '' ) . 'Msg', 'seraphinite-accelerator' ) ); } ) );
				...
				case 2:
					...
					else
					{
						...
						if( ( $pos = strpos( $error, ":" ) ) !== false )
						{
							...
							$error = rawurldecode( substr( $error, 0, $pos ) );
						}
					}
					...
					{
						if( $error === 'alreadyProcessed' || $error === 'notChanged' )
